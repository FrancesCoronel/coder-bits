...
public class WordCount {
	...
	/**
	 * getCountsArray method
	 * @param  counter [DataCounter that has E number of objects]
	 * @return array of DataCount objects each containing a unique word
	 */
	...
	{
   		DataCount[] dataCountArray = (DataCount[]) new DataCount[counter.getsize()];
   		SimpleIterator<DataCount> dataCountIt = counter.getIterator();
   		int index = 0;
   		while (dataCountIt.hasNext())
   		{
   			dataCountArray[index] = dataCountIt.next();
   			index++;
   		}
   ...
   {
		System.err.println("Usage: [-s | -o] <filename of document to analyze>");
	...
	public static void main(String[] args) 
   {
		...
		if (firstArg.equals("-s"))
			counter = new HashTable_SC(new StringComparator(), new StringHasher());
		else if (firstArg.equals("-o"))
			counter = new HashTable_OA(new StringComparator(), new StringHasher());
		else
			usage();
